...
The first thing you will want to day is examine the experiment status. Once you run this script you will need to wait until the experiment is fully booted to continue.
In the topology viewer you can see that there are 6 nodes total, 4 compute nodes, one for parallel file systems and one for networks.
You will be able to see if each node is ready and also if any of them have failed.
...
for i in range(NUM_WORKER_CORES + 1):
    node = request.RawPC("Gordon" + str(i))
